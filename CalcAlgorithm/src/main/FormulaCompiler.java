...
/**
 * FormulaCompiler converts Calc formula string into PocketXL bytes and PocketXL
 * formula bytes into Calc Formula strings
 * 
 * For converting from infix to Reverse Polish (or Postfix) notation the string
 * is converted into a vector of Tokens and then re-ordered based on a modified
 * version of the standard Infix to RPN conversion algorithms.
 * 
 * <pre>
 * Infix2Rpn(tokens)
...
public class FormulaCompiler {
	...
	//
	// /**
	// * Convert the infix expression to RPN. Note that open brackets are saved
	// onto the stack to preserve the users bracketing.
	// * <p>Also note that the open bracket following functions is not pushed
	// onto the stack - it is always implied when
	// * writing out results
	// *
	// * @param tokens The vector of tokens in Infix form
	// *
	// * @return A vector of tokens for the expression in Reverse Polish
	// Notation order
	// */
	...
	// rpnExpr.add(pt);
	// } else if (pt.isFunction() || isParamDelimiter(pt) || isOpenBrace(pt)) {
	// //Extract parameters after afunction or comma
	// evalStack.push(pt);
	// if (pt.isFunction()) {
	// iter.next();
	// }
	...
	// } else if (isCloseBrace(pt)) { //Pop off stack till you meet a function
	// or an open bracket
	// Token tmpTok = null;
	// boolean bPop = true;
	// while (bPop) {
	...
	// //if (!(isOpenBrace(tmpTok) || isParamDelimiter(tmpTok))) { //Don't
	// output brackets and commas
	...
	// while (!evalStack.isEmpty()) {
	// Token topTok = (Token)evalStack.peek();
	// if (!(isOpenBrace(topTok) || isParamDelimiter(topTok))) { //Don't output
	// brackets and commas
	// rpnExpr.add(evalStack.pop());
	// }
	// else
	...
	// while (iter.hasNext()) {
	// Token pt = (Token)iter.next();
	// Debug.log(Debug.TRACE, "Token is " + pt + " and subExprCount is " +
	// subExprCount);
	// if (isOpenBrace(pt)) {
	// subExprCount++;
	// param.add(pt);
